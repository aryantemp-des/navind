...
Integrate the React hero component below into the existing codebase. This is the **first time this prompt is being sent to Antigravity**, so do not assume that any setup, files, dependencies, directories, routes, or component integrations from this prompt already exist.

Treat this entire file as the implementation specification and execute the integration from start to finish.
...
If the existing codebase does not support these requirements, set it up appropriately using the shadcn CLI and install/configure the required Tailwind CSS and TypeScript dependencies.

Do not unnecessarily replace or restructure an existing working application.

## Component Location

First determine the project's existing default paths for components and styles.

The requested component should live at:

`/components/ui/glassmorphism-trust-hero.tsx`

If the project uses a different component path, create/use `/components/ui` so this component follows the requested shadcn structure. Update imports consistently.

## Fresh Integration Requirements

- Create every missing file and directory required by this implementation.
- Install all required npm dependencies.
- Preserve TypeScript compatibility.
- Use `lucide-react` for icons.
- Do not introduce unnecessary icon or logo libraries.
- Preserve the supplied component's visual structure, animations, layout, and behavior unless explicitly changed below.
- Make sure the component is responsive on mobile, tablet, and desktop.
- Do not remove unrelated existing application functionality.
- Ensure the component is actually rendered from an appropriate page/demo route.
- Resolve all TypeScript, import, dependency, and build errors.
- Verify the final application builds successfully after integration.

## Explicit Content Change

Replace the original hero description:
...
Do not retain the old description in the implemented component.

## Component Source

The following is the complete component and integration specification. Copy the component into the correct location and follow the integration instructions exactly.
